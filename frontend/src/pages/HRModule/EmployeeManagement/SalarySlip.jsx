...
const SalarySlip  = ({ initialEmployee = null }) => {
  ...
  // Helper: populate form from any employee object (routedEmployee or search match)
  const fillFromEmployee = (emp) => {
    const basic    = Number(emp.salary_basic   || 0);
    const hra      = Number(emp.salary_hra     || 0);
    const conveyance = Number(emp.salary_travel_allowance  || 0);
    const medical  = Number(emp.salary_medical_allowance || 0);
    const special  = Number(emp.salary_other_allowance   || 0);
    // Prefer epf_fixed_amount when set (overrides percentage-based pf)
    const pf       = Number(emp.epf_fixed_amount || emp.salary_pf || emp.employer_pf || 0);
    ...
    // employee_id field is the VARCHAR code (ed.id), used by getEmployeeSalaryHistory
    setSelectedEmployeeId(emp.employee_id || emp.emp_id || '');
  };
  ...
  // Auto-fill when navigated from Employee Management with a selected employee
  useEffect(() => {
    if (routedEmployee) fillFromEmployee(routedEmployee);
  }, [routedEmployee]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleFetchFromDB = async () => {
    if (!selectedEmployeeId) { alert('Please select an employee first.'); return; }
    setIsGenerating(true);
    try {
      const res = await salaryAPI.getEmployeeSalaryHistory(selectedEmployeeId);
      const history = res.data?.history || [];
      const [monthName, yearStr] = (formData.monthYear || '').split(' ');
      const match = history.find(r =>
        r.month?.toLowerCase() === monthName?.toLowerCase() && String(r.year) === yearStr
      ) || history[0];
      if (!match) { alert('No salary record found for the selected month. Showing latest record if available.'); return; }

      const details = (() => {
        if (!match.details) return {};
        if (typeof match.details === 'object') return match.details;
        try { return JSON.parse(match.details); } catch { return {}; }
      })();

      setFormData(prev => ({
        ...prev,
        monthYear: `${match.month || prev.monthYear.split(' ')[0]} ${match.year || prev.monthYear.split(' ')[1]}`,
        earnings: {
          basic: Number(match.basic_salary || details.basic || 0),
          hra: Number(details.hra || 0),
          conveyance: Number(details.travel_allowance || details.conveyance || 0),
          medical: Number(details.medical_allowance || 0),
          special: Number(details.other_allowance || details.special || 0),
        },
        deductions: {
          pf: Number(details.pf || 0),
          pt: Number(details.professional_tax || 0),
          tds: Number(details.tds || 0),
        },
      }));

      // If earnings are all 0 and gross exists, populate basic + remainder in special
      const earningSum = Number(match.basic_salary || 0) + Number(details.hra || 0);
      if (earningSum === 0 && Number(match.gross_salary) > 0) {
        setFormData(prev => ({
          ...prev,
          earnings: {
            ...prev.earnings,
            basic: Number(match.basic_salary || 0),
            special: Math.max(0, Number(match.gross_salary) - Number(match.basic_salary || 0)),
          },
          deductions: {
            pf: 0, pt: 0, tds: Number(match.deduction_amount || 0),
          },
        }));
      }
    } catch (err) {
      alert(err.response?.data?.message || 'Failed to fetch salary data');
    } finally {
      ...
      <div className="slip-form-container" ref={formRef}>
        ...
        <div className="form-section" style={{ paddingTop: 0 }}>
          <button
            ...
            style={{
              padding: '9px 18px', background: '#1C47C9', color: '#fff', border: 'none',
              borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: 'pointer',
              opacity: (!selectedEmployeeId || isGenerating) ? 0.5 : 1,
            }}
          >
            {isGenerating ? 'Fetching...' : 'Load Salary from Database'}
          </button>
          <div style={{ fontSize: 11, color: 'var(--theme-text-muted,#64748b)', marginTop: 6 }}>
            Auto-fills earnings &amp; deductions from the saved salary record for the selected month.
          </div>
        </div>
      ...
      <div className="slip-preview-container">
        ...
        <div className="slip-paper" id="salary-slip-content" style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          {(() => {
            ...
            return (
              <div style={{ position: 'absolute', ...posStyle, width: wmWidth, pointerEvents: 'none', zIndex: 0,
