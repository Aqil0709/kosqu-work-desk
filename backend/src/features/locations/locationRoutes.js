...
router.post('/', async (req, res) => {
  ...
  try {
    const { name, location_type, latitude, longitude, radius_meters = 100, address } = req.body;
    if (!name || !latitude || !longitude) {
      return res.status(400).json({ success: false, message: 'name, latitude and longitude are required' });
    }
    const [result] = await pool.execute(
      `INSERT INTO tb_work_locations (tenant_id, name, location_type, latitude, longitude, radius_meters, address)
       VALUES (?,?,?,?,?,?,?)`,
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const { name, location_type, latitude, longitude, radius_meters, address, is_active } = req.body;
    await pool.execute(
      `UPDATE tb_work_locations SET name=?, location_type=?, latitude=?, longitude=?, radius_meters=?, address=?, is_active=?
       WHERE id = ? AND tenant_id = ?`,
      [name, location_type, latitude, longitude, radius_meters ?? 100, address || null, is_active ?? 1, req.params.id, req.tenantId]
    );
...
// PUT /api/locations/assign/employee — assign employee to location
