...
const { ensureDeliverySchema } = require('./deliverySchema');
...
const DeliveryChallan = {
    ...
    create: async (tenantId, challanData) => {
        ...
        const [result] = await pool.execute(
            `INSERT INTO delivery_challans (
                tenant_id, client_id, project_id, service_id, challan_no, challan_date, destination, dispatched_through,
    ...
    getRecent: async (tenantId, limit = 10) => {
        ...
        const [challans] = await pool.query(
            `SELECT * FROM delivery_challans WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ${safeLimit}`,
            [tenantId]
        );
        return challans;
    }
